...
class ValidateEbikePackage implements ValidationRule
{
    /**
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        $check_exists = EbikePackage::where('id',$this->request->package)->first();
        ...
        if($check_exists){
            ...
            {                
                if($request->mobile){
                    ...
                    if($tx_user){
                        $chkinvestmentExists = EbikeInvestment::where('package_id',$check_exists->id)->where('user_id',$tx_user->id)->where('status',1)->first();

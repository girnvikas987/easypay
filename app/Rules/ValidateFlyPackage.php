...
class ValidateFlyPackage implements ValidationRule
{
    /**
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        $check_exists = FlyPackage::where('id',$this->request->package)->first();
        ...
        if($check_exists){
            ...
            if($check_exists->no_of_time=="multiple")
            {                
                if($request->mobile){
                    $tx_user = User::where('mobile',$request->mobile)->first();
                    if($tx_user){
                        $chkinvestmentExists = FlyInvestment::where('package_id',$check_exists->id)->where('user_id',$tx_user->id)->where('package_status',1)->first();
                        if($chkinvestmentExists){
                            if($chkinvestmentExists->received_amnt <'1200'){
            ...
            {   
                ...
                if($pre_id>=1){
                    ...
                    if($request->mobile){
                        $tx_user = User::where('mobile',$request->mobile)->first();
                        if($tx_user){
                            $chkinvestmentExists = FlyInvestment::where('package_id',$pre_id)->where('user_id',$tx_user->id)->where('status',1)->first();
                            if(!$chkinvestmentExists){

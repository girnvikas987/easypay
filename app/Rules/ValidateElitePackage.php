...
class ValidateElitePackage implements ValidationRule
{
    /**
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        $check_exists = SavingPackage::where('id',$this->request->package)->first();
        if($check_exists){
            ...
            $multiple=$check_exists->amount; 
                ...
                if($this->request->amount!=''){
                    ...
                    // if ($this->request->amount % $multiple != 0) {   // 👈 multiple check
            ...
            {              
                if($request->mobile){
                    ...
                    if($tx_user){
                        $chkinvestmentExists = SavingFundInvestment::where('user_id',$tx_user->id)->where('package_id',$this->request->package)->where('status',1)->first();
            ...
            {   
                ...
                if($pre_id>=1){
                    ...
                    if($request->mobile){
                        $tx_user = User::where('mobile',$request->mobile)->first();
                        if($tx_user){
                            $chkinvestmentExists = SavingFundInvestment::where('user_id',$tx_user->id)->where('package_id',$pre_id)->where('status',1)->first();
                            if(!$chkinvestmentExists){

...
class DashboardController extends Controller
{
    ...
    public function getVideos(){
        $data = Gallery::where('type','video')->get();
    ...
    if($gen){
        ...
        $data=array();
         ...
         $incomes = [
            ...
            ['name' => 'Monthly Salary', 'slug' => 'mothly_salary'],
            ['name' => 'Daily Cashback', 'slug' => 'daily_cashback'],
            ['name' => 'Reward', 'slug' => 'reward'],
            ['name' => 'Refferal Cashback', 'slug' => 'refferal_cashback'],
    ...
    public function getSponsorInfo(Request $request){

        $sponsorinfo = Team::where('user_id',$request->user()->id)->first();

        $sponsorId = $sponsorinfo->sponsor;

        if($sponsorId != 0 || $sponsorId != null){
            $userInfo = User::find($sponsorId);
             $response = [
                'success' => true,
                'data' => $userInfo
            ];
